...
### Fixed
- Exporting photos could run for many hours without producing a single file, with Photos pinned near 100% CPU and the sync stuck at 78%. Photos with no copy on your Mac — anything kept only in iCloud, including Shared Library items — were collected by asking Photos to find them one filename at a time, and Photos answers that kind of question by reading through the entire library from the beginning every single time. A first full-library export of around 28,000 photos would have spent roughly a day looking up names before writing anything at all. iCloudBridge now fetches those photos through PhotoKit, macOS's own photo library interface, which finds each one directly. Files begin arriving within seconds and the progress bar moves as they do.
- Photos that share a filename are no longer lost. A library can hold several genuinely different photos all called `IMG_1234.HEIC`, and the export matched the files Photos handed back by name alone — so all but one of each group was dropped and counted as an error. In a library of 28,480 photos, 6,156 filenames were duplicated, affecting close to 10,000 photos. Photos are now matched on their identifier, which is unique to each one, so every photo is exported exactly once.
- Photos that share a filename no longer overwrite each other in the export folder. Separately from the matching problem above, two different photos called `IMG_1234.HEIC` taken in the same month were both written to the same place, so the second silently replaced the first while iCloudBridge recorded both as exported. Each photo now gets its own file, with a number added to the name where two would otherwise clash. If an earlier export already lost photos this way, `scripts/recover_photo_exports.py` finds the affected records and clears them so the next sync exports those photos again; run it with no arguments first for a report, and add `--apply` when you are happy with what it says.
- Interrupting a photo export no longer leaves a half-written file behind. Files were written directly to their final location, so a sync stopped mid-copy left a truncated photo that looked like a finished one. Each file is now written alongside and moved into place once complete, so a photo in your export folder is always whole.
- A photo export now checks there is room before it starts, and stops immediately if the disk fills. Exporting to a folder on the same disk as your photo library needs as much free space again as the library occupies; iCloudBridge simply started copying and kept going as the disk filled, logging a separate failure for every remaining photo and leaving the disk at zero free space, which is enough to destabilise macOS itself. It now estimates the space needed up front - measuring the photos on disk, since macOS does not always record their sizes - and refuses to start with a message saying how much is needed and how much is free. If the disk fills mid-export anyway, it stops at the first failure instead of the twenty-thousandth.
- A photo export that hangs can no longer run indefinitely. There was no limit on how long iCloudBridge would wait for Photos to answer, so a stuck request simply stayed stuck — in one case for over sixteen hours. Requests now have a time limit, and a request that exceeds it is stopped so the rest of the export can carry on.
- Cloud photo exports now report real progress and survive interruption. The sync sat at exactly 78% for the entire download, however long it took, and a run that failed part-way lost everything it had done. Progress now advances as photos arrive, and each photo is recorded as it completes, so a later run resumes instead of starting over.
- Photo export totals no longer under-report. Photos skipped because an identical copy had already been exported were counted as neither exported nor failed, so the numbers at the end of a sync didn't add up to the number of photos considered.

### Changed
- The menubar app now handles Photos access on the backend's behalf. macOS grants photo library access per application, and it does not extend that grant from iCloudBridge to the Python process behind it — that process is identified to macOS as Homebrew's copy of Python, which cannot ask for photo access at all. The menubar app already holds the grant, so it now does the photo library work and passes the results back over a local, token-protected connection. Photo exports are therefore fastest with the menubar app running; with it closed, iCloudBridge falls back to the previous, much slower method.
